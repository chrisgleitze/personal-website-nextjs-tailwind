...
```text title="Install Vim from source on Linux"

Instructions on how to use Vim on Linux via the github.com/vim/vim repo.

On some distributions Linux is installed by default.
Let's make sure to uninstall a possible Vim installation.

-- uninstall Vim

sudo apt remove vim vim-runtime vim-common

-- clone the Vim repo to your local machine
...
-- the 'make' cmd creates an executable
-- according to the compile instruction in the Makefile
make

-- install it globally on your system
-- find the installed executable in /usr/local/
-- on my machine the vim executable is in /usr/local/bin/
sudo make install

Run 'vim --version' to check the release details.
It should say something like "Included patches: 1-2077".
That means it's version 9.1.2077.

Now you should be able to run 'vim' from any folder.

Regularly pull the changes made to the vim/vim repo
to your machine with the 'git pull' command.
Repeat the steps to 'make' vim.

See https://www.vim.org/git.php for more info on this way of using Vim.
```
...
(insert mode) Ctrl + o : go from insert mode to normal mode for one command, then back to insert mode
c - change
d - delete
r - replace one character
a - go into insert mode after character under cursor
y - yank (copy) text
...
_ - jump to first non-space character of the line
...
cc : change (replace) entire line (S as well)
...
dw : delete word
(insert mode) Ctrl + w : delete last word
...
d/er : deletes up to next occurrence of er

s : delete one character
S : delete line and go into insert mode
cc : delete line and go into insert mode

:r OR :read : paste content of a file into current buffer, e.g.:
:read global/collection.js : pastes content of collection.js into current buffer
:r ! [shell command] : execute shell command and paste output into current buffer
:r ! ls : lists output of ls into the buffer,
so that's names of files and directories of current folder
...
Ctrl-a and Ctrl-x de- and increases number of word under cursor
